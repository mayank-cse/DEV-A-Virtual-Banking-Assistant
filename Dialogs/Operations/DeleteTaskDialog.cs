...
using DevVirtualBankingAssistant;
...
namespace ToDoBot.Dialogs.Operations
{
    public class DeleteTaskDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> ShowTasksStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                await stepContext.Context.SendActivityAsync(MessageFactory.Text("You don't have any payment record to delete."), cancellationToken);
            ...
            await stepContext.Context.SendActivityAsync(MessageFactory.Text("Please select the payment record you want to delete."), cancellationToken);
        ...
        private async Task<DialogTurnResult> DeleteTasksStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (deleteTask)
            {
                await stepContext.Context.SendActivityAsync(MessageFactory.Text("Payment record '" + taskToDelete + "' successfully deleted."), cancellationToken);

                List<ToDoTask> toDoTasks = await _cosmosDBClient.QueryItemsAsync(User.UserID);

                if (toDoTasks.Count == 0)
                {
                    await stepContext.Context.SendActivityAsync(MessageFactory.Text("No payment record left. All your payment record are deleted."), cancellationToken);
                    return await stepContext.EndDialogAsync(null, cancellationToken);
                }

                return await stepContext.PromptAsync(nameof(ConfirmPrompt), new PromptOptions
                {
                    Prompt = MessageFactory.Text("Would you like to Delete more payment pecord?")
                }, cancellationToken);
            }
            else
            {
                await stepContext.Context.SendActivityAsync(MessageFactory.Text("Payment record '" + taskToDelete + "' could not be deleted. Either it has been already deleted or some error occurred."), cancellationToken);
                return await stepContext.EndDialogAsync(null, cancellationToken);
            }
