...
namespace DevVirtualBankingAssistant.Dialogs
{
    public class ScheduleTask : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> IntroStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            List<string> operationList = new List<string> { "Create Payment Record", "View Payment Records", "Delete Payment Record", "Exit", "Change User" };
            // Create card
            var card = new AdaptiveCard(new AdaptiveSchemaVersion(1, 0))
            {
                // Use LINQ to turn the choices into submit actions
        ...
        private async Task<DialogTurnResult> FinalStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            // Restart the main dialog with a different message the second time around
            var promptMessage = "Thank You for choosing Dev. What else can I do for you?";
            return await stepContext.ReplaceDialogAsync(InitialDialogId, promptMessage, cancellationToken);
        }
