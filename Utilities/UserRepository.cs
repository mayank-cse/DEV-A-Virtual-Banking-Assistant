...
namespace DevVirtualBankingAssistant.Utilities
{
    public class UserRepository
    {
        ...
        public async Task<bool> createTeamsMeet(string Name,string Email, string Time, string Reason, string uri)
        {
            try
            {
                HttpClient client = new HttpClient();
                ...
                HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, client.BaseAddress);
                var body = $"{{\"Name\": \"{Name}\",\"Email\": \"{Email}\",\"Time\":\"{Time}\",\"Reason\":\"{Reason}\"}}";
                var content = new StringContent(body, Encoding.UTF8, "application/json");
                request.Content = content;
                var response = await MakeRequestAsync(request, client);
                var authenticationModel = JsonConvert.DeserializeObject<AuthenticationModel>(response);
